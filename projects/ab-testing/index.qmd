...
The bootstrapped standard error is **`{python} round(boot_se, 4)`**, while the analytical standard error is **`{python} round(analytical_se, 4)`**. These two values are very close, which is reassuring because it suggests that the resampling approach and the formula-based approach are telling a similar story about the precision of the estimator.

Using the bootstrapped standard error, I construct a 95% confidence interval for $\theta$ of **[`{python} round(ci_lower, 4)`, `{python} round(ci_upper, 4)`]**. This interval gives a range of plausible values for the true difference in sign-up rates between CTA A and CTA B. Because the interval lies mostly above zero, the evidence is consistent with CTA A producing a higher sign-up rate than CTA B.
...
The test statistic is **`{python} round(z_stat, 4)`**, and the corresponding two-sided p-value is **`{python} round(p_value, 4)`**. The test statistic measures how many standard errors the observed estimate is away from the null value of zero. A small p-value indicates that such a result would be unlikely if the two CTAs truly had the same sign-up rate. In this case, the evidence supports rejecting the null and concluding that the two CTAs differ in performance.
...
The estimated treatment coefficient is **`{python} round(beta_1, 4)`**, with standard error **`{python} round(se_beta_1, 4)`**, t-statistic **`{python} round(t_beta_1, 4)`**, and p-value **`{python} round(p_beta_1, 4)`**. These values are very close to the earlier two-sample results, which is exactly what we should expect. This equivalence matters because it shows that the same regression tools used throughout applied statistics can also analyze simple A/B tests, while also extending naturally to richer experimental designs.
