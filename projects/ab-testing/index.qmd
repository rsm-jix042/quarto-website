...
first_significant_peek = np.full(n_experiments, np.nan)
final_only_flags = np.zeros(n_experiments)

for exp in range(n_experiments):
    A_null = np.random.binomial(1, pi_null, n_total)
    B_null = np.random.binomial(1, pi_null, n_total)

    theta_final = A_null.mean() - B_null.mean()
    pi_A_final = A_null.mean()
    pi_B_final = B_null.mean()
    se_final = np.sqrt(
        (pi_A_final * (1 - pi_A_final) / n_total) +
        (pi_B_final * (1 - pi_B_final) / n_total)
    )

    if se_final > 0:
        z_final = theta_final / se_final
        p_final = 2 * (1 - stats.norm.cdf(abs(z_final)))
        final_only_flags[exp] = p_final < 0.05
...
To make the comparison more concrete, I also compute the false positive rate from a fixed-horizon design where I only test once, after all 1,000 observations per group have been collected. In this simulation, the final-only false positive rate is **`{python} f"{float(final_only_false_positive_rate):.3f}"`**, while the repeated-peeking false positive rate is **`{python} f"{float(empirical_false_positive_rate):.3f}"`**. The gap between those two numbers is the practical cost of checking the same experiment over and over without adjusting the testing rule.

```{python}
comparison_df = pd.DataFrame({
    "testing_rule": ["Final test only", "Repeated peeking"],
    "false_positive_rate": [
        final_only_false_positive_rate,
        empirical_false_positive_rate
    ]
})
...
bars = plt.bar(
    comparison_df["testing_rule"],
    comparison_df["false_positive_rate"]
)
plt.axhline(0.05, linestyle="--", label="Nominal 5% level")
plt.ylabel("False positive rate")
plt.title("Final Testing vs. Repeated Peeking")
plt.ylim(0, max(comparison_df["false_positive_rate"]) + 0.05)
...
Another way to see the same problem is to plot how the false positive rate accumulates as more interim checks are allowed. The figure below shows the share of simulated experiments that would have produced at least one false positive by each peek point.

```{python}
peek_risk_df = pd.DataFrame({
    "peek_point": peek_points,
    "false_positive_rate": [
        np.mean(first_significant_peek <= point)
        for point in peek_points
    ]
})

plt.figure(figsize=(8, 5))
plt.plot(
    peek_risk_df["peek_point"],
    peek_risk_df["false_positive_rate"],
    marker="o",
    label="Repeated peeking"
)
plt.axhline(0.05, linestyle="--", label="Nominal 5% level")
plt.xlabel("Observations per group at each peek")
plt.ylabel("Cumulative false positive rate")
plt.title("False Positives Accumulate with Repeated Peeking")
plt.legend()
plt.show()
```

The upward-sloping line makes the peeking problem easier to diagnose. Even though each individual test uses a 5% cutoff, the chance of getting at least one false positive increases as the number of interim checks grows.
