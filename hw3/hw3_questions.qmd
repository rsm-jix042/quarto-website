...
coefficient_labels <- c(
  "(Intercept)" = "Intercept",
  "age" = "Firm age",
  "I(age^2)" = "Firm age squared",
  "factor(region)Northeast" = "Region: Northeast",
  "factor(region)Northwest" = "Region: Northwest",
  "factor(region)South" = "Region: South",
  "factor(region)Southwest" = "Region: Southwest",
  "factor(iscustomer)1" = "Blueprinty customer"
)

clean_coefficient_names <- function(x) {
  unname(ifelse(x %in% names(coefficient_labels), coefficient_labels[x], x))
}
...
results_df <- data.frame(
  Variable = clean_coefficient_names(colnames(X)),
...
comparison_df <- data.frame(
  Variable = clean_coefficient_names(names(glm_coef)),
  MLE_Coef = beta_mle,
...
- **Blueprinty Customer:** This is our main estimate of interest. A positive coefficient suggests Blueprinty customers get more patents even after controlling for age and region.
...
A useful diagnostic for count models is whether the data vary more than a strict Poisson model expects. In a Poisson model, the conditional mean and variance are assumed to be equal. If the variance is much larger than the mean, the coefficient estimates can still be informative, but the standard errors may be too optimistic.

```{r}
#| echo: false

pearson_dispersion <- sum(diagnostics$pearson_residuals^2) / glm_model$df.residual

overdispersion_note <- case_when(
  pearson_dispersion > 1.5 ~ "Meaningful overdispersion",
  pearson_dispersion > 1.2 ~ "Mild overdispersion",
  TRUE ~ "Close to the Poisson assumption"
)

data.frame(
  Metric = c("Pearson dispersion", "Interpretation"),
  Value = c(round(pearson_dispersion, 2), overdispersion_note)
) %>%
  knitr::kable(row.names = FALSE)
```

The Pearson dispersion estimate is `r round(pearson_dispersion, 2)`, which suggests mild extra variation beyond the strict Poisson assumption. That does not overturn the main pattern, but it is a reason to treat the standard errors as approximate. In a production analysis, I would also report quasi-Poisson, negative binomial, or robust-standard-error results.
...
### A Typical-Firm Translation

The average treatment effect is useful, but it is easier to understand the result with one concrete counterfactual example. Holding firm age and region fixed, the model asks: what would the same firm be expected to receive as a non-customer versus as a Blueprinty customer?

```{r}
#| echo: false

typical_age <- median(blueprinty_data$age)
typical_region <- blueprinty_data %>%
  count(region, sort = TRUE) %>%
  slice(1) %>%
  pull(region)

typical_firm <- data.frame(
  age = typical_age,
  region = typical_region,
  iscustomer = c(0, 1)
)

typical_prediction <- as.numeric(
  predict(glm_model, newdata = typical_firm, type = "response")
)

typical_effect <- typical_prediction[2] - typical_prediction[1]
typical_lift <- (typical_prediction[2] / typical_prediction[1] - 1) * 100

data.frame(
  Scenario = c("Same firm as a non-customer", "Same firm as a Blueprinty customer"),
  "Predicted patents over 5 years" = round(typical_prediction, 2),
  "Difference vs. non-customer" = c("--", paste0("+", round(typical_effect, 2))),
  "Lift" = c("--", paste0(round(typical_lift, 1), "%")),
  check.names = FALSE
) %>%
  knitr::kable(row.names = FALSE)
```

For a firm at the sample median age (`r round(typical_age, 1)` years) in the most common region (`r typical_region`), the model predicts about `r round(typical_effect, 2)` additional patents over five years if the firm is treated as a Blueprinty customer. This turns the regression coefficient into a more business-readable statement.
...
### What Blueprinty Can Say

A careful and defensible marketing statement would be: **among firms of similar age and region, Blueprinty customers are associated with higher expected patent approvals over five years.** That is a stronger statement than a raw comparison, because it adjusts for observable differences between customers and non-customers.

The statement I would avoid is: **Blueprinty's software causes firms to receive more patents.** This dataset is observational, so it supports a strong association, not a clean causal claim.
